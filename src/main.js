...
class AcodePlugin {
  ...
  async getDir() {
    ...
    const myFile = await fileBrowser('folder', 'Open a sample file', true);
    return myFile;
  }
  ...
  async getDirToSaveTheFile() {
    ...
    if (selectedPath.includes("com.termux.documents") && !selectedPath.includes("::/data/data/com.termux/files/home/")) {
      selectedPath = selectedPath + `::/data/data/com.termux/files/home/${options[1]}`;
    }
  ...
  async clone(remotURL) {
    try {
      ...
      if (remotURL) {
        ...
        const cloneload = loader.create('CLONE REPOSITORY', "Cloning Remote", {
          timeout: 5000,
